...
void modified_gram_schmidt(double **V, double **X, double **R, int m, int n)
{
	double **Y;
	Y = new double *[m];
	for (int i = 0; i<m; ++i)
	...
	for(int j = 1; j < m; ++j) // for all vectors
	{
		for (int k = 0; k< n; ++k)
			{
				Y[j][k] = X[j][k];
			}
			for (int l = 0; l < j; ++l) // removing compoinent other vectors
				{
					R[l][j] = dot_product(V[l], Y[j], n);
					for (int k = 0; k<n; ++k)
						{
							Y[j][k] = Y[j][k] - R[l][j]*V[l][k];  // equalising for all items
						}	
				}
...
int main(){
	...
	modified_gram_schmidt(v, x, r, m, n);
	print_vectors(v, m, n);
	print_vectors(r, m, m);
}
